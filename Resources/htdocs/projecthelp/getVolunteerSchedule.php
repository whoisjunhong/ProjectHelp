...
$query_rsGetSchedule = sprintf("SELECT * FROM scheduled INNER JOIN request on scheduled.requestId = request.requestId WHERE scheduled.aceptee = %s", $currentUser);
$rsSchedule = mysqli_query($connDB, $query_rsGetSchedule);
$response["msgType"] = $requestType;

if(mysqli_num_rows($rsSchedule) > 0){
	while ($row_RequestData = mysqli_fetch_assoc($rsSchedule)) {
    $response["volunteerScheduledDetails"][] = array(
